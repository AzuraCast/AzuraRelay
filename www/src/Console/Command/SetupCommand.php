...
class SetupCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $question->setValidator(function ($value) use ($baseUrl) {
            ...
            if (0 === count($relays)) {
                throw new \RuntimeException(
                    'No relayable streams were found on the remote server. Make sure your account ' .
                    'has permission to "Manage Broadcasting" for the station you want to relay.'
                );
            }
        ...
        $question = new Question\ConfirmationQuestion(
            'Show This Relay to AzuraCast Listeners?',
            getenv('AZURARELAY_IS_PUBLIC') ?? true
        );
